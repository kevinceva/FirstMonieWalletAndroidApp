    ...
    protected Weather parseWidgetJson(String result, Context context) {
        try {
            ...
            if (lastUpdateTimeInMillis < 0) {
                ...
                lastUpdate = context.getString(R.string.last_update_widget, WeatherActivity.formatTimeWithDayIfNotToday(context, lastUpdateTimeInMillis));
            }
            ...
            widgetWeather.setWind(context.getString(R.string.wind) + ": " + new DecimalFormat("#.0").format(wind) + " " + localize(sp, context, "speedUnit", "m/s")
                    + (widgetWeather.isWindDirectionAvailable() ? " " + WeatherActivity.getWindDirectionString(sp, context, widgetWeather) : ""));
